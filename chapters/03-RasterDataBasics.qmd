# Raster Data Basics with `terra` {#sec-raster-basics}

```{r}
#| eval: false
#| echo: false
# This is for debugging the code on the local computer 
...
# This code is for obtaining data used in the chapter
...
```{r}
terra::ext(IA_cdl_ml_sr)
```

You can access the dimensions of a `SpatRaster` object as follows: 

```{r}
#--- number of columns ---#
terra::ncol(IA_cdl_ml_sr)

#--- number of rows ---#
terra::nrow(IA_cdl_ml_sr)

#--- number of layers ---#
terra::nlyr(IA_cdl_ml_sr)

#--- number of cells ---#
terra::ncell(IA_cdl_ml_sr) 
```
...
#| eval: false
#--- terra::values ---#
values_from_ml_sr <- terra::values(IA_cdl_ml_sr)

#--- take a look ---#
head(values_from_ml_sr)
```

```{r values-show}
#| echo: false
# saveRDS(head(values_from_ml_sr), "Data/values-from-rs.rds")
...
### Subset (access particular layers)
...
```{r}
subset(IA_cdl_ml_sr, "IA_cdl_2016")
```

```{r }
#--- second layer ---#
subset(IA_cdl_ml_sr, 2)
```

If you are accessing a single layer, you can use a familiar-looking syntax using `$` like below:

```{r}
IA_cdl_ml_sr$IA_cdl_2016
```
...
Here, we created a new layer called `new_layer` to `IA_cdl_ml_sr`, where the values stored in the new layer are simply the valuesn stored in the `IA_cdl_2016` layer.
...
Let's first import NIR and RED.

```{r}
#| cache: false
#| label: read-nir-red
NIR_and_RED <- terra::rast(c("Data/NIR.tif", "Data/RED.tif"))
```
...
#| cache: false
#| label: calc-ndvi
NIR_and_RED$NDVI <- (NIR_and_RED$NIR - NIR_and_RED$RED)/(NIR_and_RED$NIR + NIR_and_RED$RED)
```

Now, we have a layer named `NDVI` in `NIR_and_RED`.

:::{.callout-note}
The `tidyterra` package can help you make the codes a bit easier to read by allowing the user to use `dplyr` verbs. For example, the above code can be written as below in place:

```{r}
dplyr::mutate(NIR_and_RED, NDVI = (NIR - RED)/(NIR + RED))
```

There are other `dplyr` verbs (not all) you can use to `SpatRaster`. See [here]() for more detailed explanations of how `tidyterra` works. We will use the package also to visualize `SpatRaster` objected later in @sec-create-maps.
:::

You can also apply mathematical functions to the layers like below^[The calculated numers are not meaningful. This is just a demonstration of using mathematical functions.]:

```{r}
log(NIR_and_RED$NIR) + NIR_and_RED$RED^2
```
...
Second, once a raster object is converted to point `sf` data, it becomes impossible to weight cell values based on their degree of overlap with the target polygons. While working with a `data.frame` may be appealing due to their familiarity, the conversion is often unnecessary and inefficient^[If you know of cases where converting to a `data.frame` is beneficial or necessary, please let me know, and I will include them here.].
:::
