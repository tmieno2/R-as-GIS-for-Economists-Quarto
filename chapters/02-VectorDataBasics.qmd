...
# Vector Data Basics with `sf` {#sec-vector-basics}
...
To test your knowledge, you can work on coding exercises at @sec-sf-basics-exercises.
...
## Projection with a different Coordinate Reference Systems {#sec-projection-sf}
...
## Turning a `data.frame` of points into an `sf` {#sec-df-to-sf}
...
### `sf::st_distance()` {#sec-distance}

`sf::st_distance` calculates distances between `sfg`s. Its output is the matrix of distances whose $i,j$ element is the distance between the $i$th `sfg` of the first `sf` and $j$th `sfg` of the second `sf`. The following code find the distance between the first 10 wells in `urnrd_wells_sf`.
...
When you focus on a single county, you can see that county borders in this data are fairly detailed. Let's look at the Cook county (@fig-cook-county-border).
...
```{r cook-simplified}
Cook_simplified <- sf::st_simplify(Cook, dTolerance = 1000)
```
...
## Exercises {#sec-sf-basics-exercises}
...
After running `data(fairway_grid)`, check the CRS of `fairway_grid` and then transform it using WGS84 (look up the EPSG code for WGS84).^[Hint: see @sec-projection-sf]
...
### Exercise 2: Turn a `data.frame` of points into an `sf`

Using the `LAT` (latitude) and `LNG` (longitude) columns in `mower_sensor`, turn the `tibble` into an `sf`, and then assign the CRS of WGS 84 using its EPSG code.^[Hint: see @sec-df-to-sf] 
...
### Exercise 3: Centroid and distance

Find the centroid of the polygons in `fairway_grid` and then calculate the distance between the centroids. Which polygon is the closest to the first polygon recorded in `fairway_grid`?^[Hint: see @sec-centroid and @sec-distance]
