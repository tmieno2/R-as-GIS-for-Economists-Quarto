...
**Wells in Kansas as points** (@fig-ks-wells-only)
...
::: {.column-margin}
```{r}
#| label: fig-ks-wells-only
#| fig-cap: "Map of wells in Kansas"
#| context: output
ggplot(gw_KS_sf) +
  geom_sf(size = 0.5) +
  theme_void()
...
**Kansas county border as polygons** (@fig-ks-borders-only)
...
::: {.column-margin}
```{r}
#| label: fig-ks-borders-only
#| fig-cap: "Map of Kansas county borders" 
#| code-fold: true
ggplot(KS_county) + 
  geom_sf() +
  theme_void()
...
**Railroads as lines** (@fig-railroads-only)
...
Right now, gray color is assigned to the cells with NA. To make them disappear, add `na.value = "transparent"` in the `scale_fill_*()` function like below:

```{r}
#| label: viz-spatraster-facet-style-na
ggplot() +
  geom_spatraster(data = tmax_Jan_09_sr) +
  facet_wrap(~ lyr, ncol = 2) +
...
## Combine `geom_sf()` and `geom_spatraster()`/`geom_stars()` layers

You can easily combine layers based on vector and raster data. Let's crop the tmax data to Kansas and create a map of tmax values displayed on top of the Kansas county borders.
...
Mixing either `geom_spatraster()` or `geom_stars()` layers with `geom_sf()` layers is no different from combining various non-spatial `geom_*()` layers within the `ggplot2` framework. The example below demonstrates how to mix them:
...
Let's first create a map of the area of interest (@fig-map-interest).

```{r}
#| label: fig-map-interest
#| fig-cap: "Map of the area of interest"
...
Create a map of the region that include the area of interest (@fig-map-large) and then turn it into a `grob`.

```{r}
#| label: fig-map-large
#| fig-cap: "Map of the region that include the area of interest"
...
```{r}
#| label: inset-annotation-custom-1
g_inset +
...
```{r}
#| label: inset-annotation-custom-2
g_inset +
  annotation_custom(grob_aoi,
    xmin = 0, xmax = 0.8, ymin = 0,
    ymax = 1
...
```{r}
#| label: add-void-custom-complete
g_inset +
  annotation_custom(grob_aoi,
    xmin = 0, xmax = 0.8, ymin = 0,
    ymax = 1
